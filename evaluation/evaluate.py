...
def extract_embed_bge_vl_t2i(model, data_list, mode='image', t2i_prompt=''):
    embed_all = None
    with torch.no_grad():
        if mode == 'image':
            batch_size = 32
            for start_index in tqdm(range(0, len(data_list), batch_size)):
                embed_batch = model.encode([dict(image=image) for image in data_list[start_index: start_index+batch_size]], convert_to_tensor=True)
                if embed_all == None:
            ...
            batch_size = 32
            for start_index in tqdm(range(0, len(data_list), batch_size)):
                embed_batch = model.encode([dict(text=text, prompt=t2i_prompt) for text in data_list[start_index: start_index+batch_size]], convert_to_tensor=True)
                if embed_all == None:
                    embed_all = embed_batch
                else:
                    embed_all = torch.cat([embed_all, embed_batch], dim=0)
...
def extract_embed_bge_vl_i2t(model, data_list, mode='image', i2t_prompt=''):
    embed_all = None
    with torch.no_grad():
        if mode == 'image':
            batch_size = 32
            for start_index in tqdm(range(0, len(data_list), batch_size)):
                embed_batch = model.encode([dict(image=image, prompt=i2t_prompt) for image in data_list[start_index: start_index+batch_size]], convert_to_tensor=True)
                if embed_all == None:
                    embed_all = embed_batch
                else:
                    embed_all = torch.cat([embed_all, embed_batch], dim=0)
            ...
            batch_size = 32
            for start_index in tqdm(range(0, len(data_list), batch_size)):
                embed_batch = model.encode([dict(text=text) for text in data_list[start_index: start_index+batch_size]], convert_to_tensor=True)
                if embed_all == None:
                    embed_all = embed_batch
                else:
                    embed_all = torch.cat([embed_all, embed_batch], dim=0)
...
parser.add_argument("--model_path", type=str, default="/path/to/gme-Qwen2-VL-7B-Instruct", help="do predict")
...
image_embeds = extract_embed_bge_vl_i2t(model, image_list, mode='image', i2t_prompt=i2t_prompt)
print('using: ', time.time()-start_time)
print('========================Extract embeds for texts========================')
start_time = time.time()
text_embeds = extract_embed_bge_vl_i2t(model, caption_list, mode='text', i2t_prompt=i2t_prompt)
print('using: ', time.time()-start_time)
...
start_time = time.time()
print('========================Extract embeds for images========================')
image_embeds = extract_embed_bge_vl_t2i(model, image_list, mode='image', t2i_prompt=t2i_prompt)
print('using: ', time.time()-start_time)
print('========================Extract embeds for texts========================')
start_time = time.time()
text_embeds = extract_embed_bge_vl_t2i(model, caption_list, mode='text', t2i_prompt=t2i_prompt)
print('using: ', time.time()-start_time)
